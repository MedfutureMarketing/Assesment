...
import Locicon from "../assets/home/location.png"; // Ensure correct path
  ...
  const JobCard = ({ job }) => (
    <div className="relative bg-white shrink-0 w-[90vw] sm:w-full sm:max-w-3xl p-4 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 border border-gray-50 mx-2 sm:mx-0">
      ...
      <div className="flex flex-col gap-2 text-left pr-24">
        ...
        <p className="text-sm text-gray-500">{job.company_name || "Company not specified"}</p>
      </div>
  ...
  return (
    <div className="w-full px-4 py-6">
      {loading ? (
        ...
        <div
          className="
            flex sm:flex-col sm:items-center 
            gap-4 
        ...
        >
          {jobs.map((job) => (
            <div className="snap-start" key={job.id}>
              <JobCard job={job} />
            </div>
          ))}
        </div>
